...
const YTPlayers = (() => {
  ...
  function playOne(playerIndex, player, videoId, startSeconds, playSeconds, onStart, onTick) {
    return new Promise((resolve) => {
      ...
      const resumeTimer = () => {
        ...
        // audio-player.js側と同様、カウントダウンUIをこの実測残り時間で
        // 駆動できるよう都度伝える(省略時は呼び出し側の壁時計表示のまま)。
        if (onTick) {
  ...
  return {
    ...
    // onTick(remainingMs): audio-player.js側と同様、先頭の曲(videoIds[0])の実際の
    // 残り再生時間が変化するたびに呼ばれる。省略可。
    // 戻り値: [{error:boolean}, ...] videoIdsと同じ順
    async playSegments(videoIds, startSecondsList, playSeconds, onStart, startPlayerIndex = 0, onTick) {
      ...
      const tasks = videoIds.map((vid, i) => {
        ...
        return playOne(idx, players[idx], vid, startSecondsList[i], playSeconds, handleStart, i === 0 ? onTick : undefined);
      });
